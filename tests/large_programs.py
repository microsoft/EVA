...
class LargePrograms(EvaTestCase):
    ...
    def test_regression(self):
        ...
        linreg_inputs = {
            "e": [(linreg.vec_size - i) * 0.001 for i in range(linreg.vec_size)]
        }
        for i in range(p):
        ...
        for j in range(k):
            multireg_inputs[f"e{j}"] = [
                (multireg.vec_size - i) * j * 0.001 for i in range(multireg.vec_size)
            ]

        compiler = CKKSCompiler(config={"warn_vec_size": "false"})

        for prog, inputs in [
            (linreg, linreg_inputs),
